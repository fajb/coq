...
| PX (p2, i, q0) ->
  let twoPQ =
    pmul cO cI cadd cmul ceqb p2 (mkPinj XH (pmulC cO cI cmul ceqb q0 (cadd cI cI)))
  in
...
(** val mapX :
    (kind -> 'a2 -> 'a2) -> kind -> ('a1, 'a2, 'a3, 'a4) gFormula -> ('a1, 'a2, 'a3,
    'a4) gFormula **)
...
(** val map_bformula :
    kind -> ('a1 -> 'a2) -> ('a1, 'a3, 'a4, 'a5) gFormula -> ('a2, 'a3, 'a4, 'a5)
    gFormula **)
...
(** val mk_and :
    ('a2 -> bool) -> ('a2 -> 'a2 -> 'a2 option) -> (bool -> kind -> ('a1, 'a3, 'a4,
    'a5) tFormula -> ('a2, 'a3) cnf) -> kind -> bool -> ('a1, 'a3, 'a4, 'a5) tFormula
...
let mk_eq unsat deduce rEC k pol0 f1 f2 =
...
let rec negate_formula _ = function
| TT k0 -> FF k0
| FF k0 -> TT k0
| X (k0, _) -> TT k0
| A (k0, _, _) -> TT k0
| AND (k0, f1, f2) -> OR (k0, (negate_formula k0 f1), (negate_formula k0 f2))
| OR (k0, f1, _) -> negate_formula k0 f1
| NOT (_, e) -> e
| IMPL (k0, f1, _, f2) ->
  if is_FF k0 f2 then f1 else AND (k0, f1, (negate_formula k0 f2))
| IFF (k0, _, _) -> TT k0
| EQ (_, _) -> TT IsProp

(** val xcnf :
    ('a2 -> bool) -> ('a2 -> 'a2 -> 'a2 option) -> (kind -> 'a1 -> 'a3 -> ('a2, 'a3)
    cnf) -> (kind -> 'a1 -> 'a3 -> ('a2, 'a3) cnf) -> bool -> kind -> ('a1, 'a3, 'a4,
    'a5) tFormula -> ('a2, 'a3) cnf **)

let rec xcnf unsat deduce normalise1 negate0 pol0 k = function
| TT _ -> if pol0 then cnf_tt else cnf_ff
| FF _ -> if pol0 then cnf_ff else cnf_tt
| X (_, _) -> cnf_ff
| A (_, x, t0) -> if pol0 then normalise1 k x t0 else negate0 k x t0
| AND (k0, e1, e2) ->
  mk_and unsat deduce (fun x x0 x1 -> xcnf unsat deduce normalise1 negate0 x x0 x1) k0
    pol0 e1 e2
| OR (k0, e1, e2) ->
  mk_or unsat deduce (fun x x0 x1 -> xcnf unsat deduce normalise1 negate0 x x0 x1) k0
    pol0 e1 e2
| NOT (k0, e) -> xcnf unsat deduce normalise1 negate0 (negb pol0) k0 e
| IMPL (k0, e1, _, e2) ->
  mk_impl unsat deduce (fun x x0 x1 -> xcnf unsat deduce normalise1 negate0 x x0 x1)
    k0 pol0 e1 e2
| IFF (k0, e1, e2) ->
  if pol0
  then and_cnf_opt
         ...
         (mk_impl unsat deduce (fun x x0 x1 ->
           xcnf unsat deduce normalise1 negate0 x x0 x1) k0 pol0 e2 e1)
  else or_cnf_opt unsat deduce
         (mk_impl unsat deduce (fun x x0 x1 ->
           xcnf unsat deduce normalise1 negate0 x x0 x1) k0 pol0 e1 e2)
         (mk_impl unsat deduce (fun x x0 x1 ->
           xcnf unsat deduce normalise1 negate0 x x0 x1) k0 pol0 e2 e1)
| EQ (e1, e2) ->
  (match is_bool IsBool e2 with
   | Some isb ->
     xcnf unsat deduce normalise1 negate0 (if isb then pol0 else negb pol0) IsBool e1
   | None ->
     mk_eq unsat deduce (fun x x0 x1 -> xcnf unsat deduce normalise1 negate0 x x0 x1)
       IsBool pol0 e1 e2)
...
(** val rxcnf_and_cnf :
    ('a1 -> bool) -> ('a1 -> 'a1 -> 'a1 option) -> bool -> (('a1, 'a2) cnf * 'a2 list)
    -> (('a1, 'a2) cnf * 'a2 list) -> ('a1, 'a2) cnf * 'a2 list **)

let rxcnf_and_cnf unsat deduce polarity c1 c2 =
  let e1,t1 = c1 in
  let e2,t2 = c2 in
  if polarity
  then (and_cnf_opt e1 e2),(rev_append t1 t2)
  else let f',t' = ror_cnf_opt unsat deduce e1 e2 in
       f',(rev_append t1 (rev_append t2 t'))

(** val rxcnf_and :
    ('a2 -> bool) -> ('a2 -> 'a2 -> 'a2 option) -> (bool -> kind -> ('a1, 'a3, 'a4,
    'a5) tFormula -> ('a2, 'a3) cnf * 'a3 list) -> bool -> kind -> ('a1, 'a3, 'a4,
    'a5) tFormula -> ('a1, 'a3, 'a4, 'a5) tFormula -> ('a2, 'a3) cnf * 'a3 list **)

let rxcnf_and unsat deduce rXCNF polarity k e1 e2 =
  rxcnf_and_cnf unsat deduce polarity (rXCNF polarity k e1) (rXCNF polarity k e2)
...
let rxcnf_or unsat deduce rXCNF polarity k e1 e2 =
  rxcnf_and_cnf unsat deduce (negb polarity) (rXCNF polarity k e1)
    (rXCNF polarity k e2)
...
let rxcnf_eq unsat deduce rXCNF polarity k e1 e2 =
...
let rec rxcnf unsat deduce normalise1 negate0 polarity k = function
| TT _ -> if polarity then cnf_tt,[] else cnf_ff,[]
| FF _ -> if polarity then cnf_ff,[] else cnf_tt,[]
| X (_, _) -> cnf_ff,[]
| A (_, x, t0) -> ratom (if polarity then normalise1 k x t0 else negate0 k x t0) t0
| AND (k0, e1, e2) ->
  rxcnf_and unsat deduce (fun x x0 x1 ->
    rxcnf unsat deduce normalise1 negate0 x x0 x1) polarity k0 e1 e2
| OR (k0, e1, e2) ->
  rxcnf_or unsat deduce (fun x x0 x1 -> rxcnf unsat deduce normalise1 negate0 x x0 x1)
    polarity k0 e1 e2
...
| IMPL (k0, e1, _, e2) ->
  rxcnf_impl unsat deduce (fun x x0 x1 ->
    rxcnf unsat deduce normalise1 negate0 x x0 x1) polarity k0 e1 e2
| IFF (k0, e1, e2) ->
  rxcnf_and_cnf unsat deduce polarity
    (rxcnf_impl unsat deduce (fun x x0 x1 ->
      rxcnf unsat deduce normalise1 negate0 x x0 x1) polarity k0 e1 e2)
    (rxcnf_impl unsat deduce (fun x x0 x1 ->
      rxcnf unsat deduce normalise1 negate0 x x0 x1) polarity k0 e2 e1)
| EQ (e1, e2) ->
  rxcnf_eq unsat deduce (fun x x0 x1 -> rxcnf unsat deduce normalise1 negate0 x x0 x1)
    polarity IsBool e1 e2
...
(** val abs_and :
    ('a1, 'a2, 'a3) to_constrT -> kind -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2,
    'a3, 'a4) tFormula -> (kind -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3,
...
(** val abs_not :
    ('a1, 'a2, 'a3) to_constrT -> kind -> ('a1, 'a2, 'a3, 'a4) tFormula -> (kind ->
    ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula) -> ('a1, 'a3, 'a2,
    'a4) gFormula **)
...
(** val mk_arrow :
    'a4 option -> kind -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4)
...
(** val abst_simpl :
    ('a1, 'a2, 'a3) to_constrT -> ('a2 -> bool) -> kind -> ('a1, 'a2, 'a3, 'a4)
    tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula **)

let rec abst_simpl to_constr needA _ = function
| A (k, x, t0) -> if needA t0 then A (k, x, t0) else X (k, (to_constr.mkA k x t0))
| AND (k0, f1, f2) ->
  AND (k0, (abst_simpl to_constr needA k0 f1), (abst_simpl to_constr needA k0 f2))
| OR (k0, f1, f2) ->
  OR (k0, (abst_simpl to_constr needA k0 f1), (abst_simpl to_constr needA k0 f2))
| NOT (k0, f0) -> NOT (k0, (abst_simpl to_constr needA k0 f0))
| IMPL (k0, f1, o, f2) ->
  IMPL (k0, (abst_simpl to_constr needA k0 f1), o, (abst_simpl to_constr needA k0 f2))
| IFF (k0, f1, f2) ->
  IFF (k0, (abst_simpl to_constr needA k0 f1), (abst_simpl to_constr needA k0 f2))
| EQ (f1, f2) ->
  EQ ((abst_simpl to_constr needA IsBool f1), (abst_simpl to_constr needA IsBool f2))
| x -> x

(** val abst_and :
    ('a1, 'a2, 'a3) to_constrT -> (bool -> kind -> ('a1, 'a2, 'a3, 'a4) tFormula ->
    ('a1, 'a2, 'a3, 'a4) tFormula) -> bool -> kind -> ('a1, 'a2, 'a3, 'a4) tFormula ->
    ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula **)

let abst_and to_constr rEC pol0 k f1 f2 =
  if pol0
  then abs_and to_constr k (rEC pol0 k f1) (rEC pol0 k f2) (fun x x0 x1 -> AND (x, x0,
         x1))
  else abs_or to_constr k (rEC pol0 k f1) (rEC pol0 k f2) (fun x x0 x1 -> AND (x, x0,
         x1))
...
let abst_or to_constr rEC pol0 k f1 f2 =
  if pol0
  then abs_or to_constr k (rEC pol0 k f1) (rEC pol0 k f2) (fun x x0 x1 -> OR (x, x0,
         x1))
  else abs_and to_constr k (rEC pol0 k f1) (rEC pol0 k f2) (fun x x0 x1 -> OR (x, x0,
         x1))

(** val abst_impl :
    ('a1, 'a2, 'a3) to_constrT -> (bool -> kind -> ('a1, 'a2, 'a3, 'a4) tFormula ->
    ('a1, 'a2, 'a3, 'a4) tFormula) -> bool -> 'a4 option -> kind -> ('a1, 'a2, 'a3,
    'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula **)
...
(** val abs_iff :
    kind -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1,
    'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4)
    tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula **)

let abs_iff _ _ _ _ _ def =
  def

(** val abst_iff :
    ('a1, 'a2, 'a3) to_constrT -> ('a2 -> bool) -> (bool -> kind -> ('a1, 'a2, 'a3,
    'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula) -> bool -> kind -> ('a1, 'a2, 'a3,
    'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula **)

let abst_iff to_constr needA rEC pol0 k f1 f2 =
  abs_iff k (rEC (negb pol0) k f1) (rEC false k f2) (rEC pol0 k f1) (rEC true k f2)
    (IFF (k, (abst_simpl to_constr needA k f1), (abst_simpl to_constr needA k f2)))

(** val abs_eq :
    ('a1, 'a2, 'a3) to_constrT -> kind -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2,
    'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4)
    tFormula -> kind -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula **)

let abs_eq to_constr k nf1 ff2 f1 tf2 r def =
...
(** val abst_eq :
    ('a1, 'a2, 'a3) to_constrT -> ('a2 -> bool) -> (bool -> kind -> ('a1, 'a2, 'a3,
    'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula) -> bool -> ('a1, 'a2, 'a3, 'a4)
    tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula -> ('a1, 'a2, 'a3, 'a4) tFormula **)

let abst_eq to_constr needA rEC pol0 f1 f2 =
  abs_eq to_constr IsBool (rEC (negb pol0) IsBool f1) (rEC false IsBool f2)
...
| NOT (k0, f0) ->
  abs_not to_constr k0 (abst_form to_constr needA (negb pol0) k0 f0) (fun x x0 -> NOT
    (x, x0))
| IMPL (k0, f1, o, f2) ->
...
(** val tauto_checker :
    ('a2 -> bool) -> ('a2 -> 'a2 -> 'a2 option) -> (kind -> 'a1 -> 'a3 -> ('a2, 'a3)
    cnf) -> (kind -> 'a1 -> 'a3 -> ('a2, 'a3) cnf) -> (('a2 * 'a3) list -> 'a4 ->
...
(** val cnf_of_list :
    'a1 -> ('a1 -> 'a1 -> bool) -> ('a1 -> 'a1 -> bool) -> 'a1 nFormula list -> 'a2 ->
    ('a1 nFormula, 'a2) cnf **)

let cnf_of_list cO ceqb cleb l tg =
  fold_right (fun x acc ->
    if check_inconsistent cO ceqb cleb x then acc else ((x,tg)::[])::acc) cnf_tt l

(** val cnf_normalise :
    'a1 -> 'a1 -> ('a1 -> 'a1 -> 'a1) -> ('a1 -> 'a1 -> 'a1) -> ('a1 -> 'a1 -> 'a1) ->
    ('a1 -> 'a1) -> ('a1 -> 'a1 -> bool) -> ('a1 -> 'a1 -> bool) -> 'a1 formula -> 'a2
    -> ('a1 nFormula, 'a2) cnf **)
...
let negate _ t0 tg =
  let f = xnnormalise t0 in if zunsat f then cnf_tt else cnf_of_list0 tg (xnegate0 f)

(** val cnfZ :
    kind -> (z formula, 'a1, 'a2, 'a3) tFormula -> (z nFormula, 'a1) cnf * 'a1 list **)
...
| PX (p2, _, q0) ->
  let g1,c1 = zgcd_pol p2 in let g2,c2 = zgcd_pol q0 in (zgcdM (zgcdM g1 c1) g2),c2
...
| ExProof (x, prf) ->
  ...
  then let z0 = Coq_Pos.succ fr in
       ...
       let post = xnnormalise (bound_var t0) in zChecker (nfx::(posz::(post::l))) prf
  else false

(** val zTautoChecker : z formula bFormula -> zArithProof list -> bool **)

let zTautoChecker f w =
  tauto_checker zunsat zdeduce normalise0 negate (fun cl -> zChecker (map fst cl)) f w
...
let qWeakChecker =
  check_normalised_formulas { qnum = Z0; qden = XH } { qnum = (Zpos XH); qden = XH }
    qplus qmult qeq_bool qle_bool

(** val qnormalise : kind -> q formula -> 'a1 -> (q nFormula, 'a1) cnf **)

let qnormalise _ =
  cnf_normalise { qnum = Z0; qden = XH } { qnum = (Zpos XH); qden = XH } qplus qmult
    qminus qopp qeq_bool qle_bool

(** val qnegate : kind -> q formula -> 'a1 -> (q nFormula, 'a1) cnf **)

let qnegate _ =
  cnf_negate { qnum = Z0; qden = XH } { qnum = (Zpos XH); qden = XH } qplus qmult
    qminus qopp qeq_bool qle_bool
...
let normQ =
  norm { qnum = Z0; qden = XH } { qnum = (Zpos XH); qden = XH } qplus qmult qminus
    qopp qeq_bool

(** val cnfQ :
    kind -> bool -> (q formula, 'a1, 'a2, 'a3) tFormula -> (q nFormula, 'a1) cnf * 'a1
    list **)

let cnfQ k b f =
  rxcnf qunsat qdeduce qnormalise qnegate b k f
...
let rnormalise t0 tg =
  cnf_normalise { qnum = Z0; qden = XH } { qnum = (Zpos XH); qden = XH } qplus qmult
    qminus qopp qeq_bool qle_bool t0 tg

(** val rnormalise_kind : kind -> q formula -> 'a1 -> (q nFormula, 'a1) cnf **)

let rnormalise_kind = function
| IsProp -> rnormalise
| IsBool -> (fun _ _ -> []::[])
...
let rTautoChecker f w =
  tauto_checker runsat rdeduce rnormalise_kind rnegate_kind (fun cl ->
    rWeakChecker (map fst cl)) (map_bformula IsProp (map_Formula q_of_Rcst) f) w
