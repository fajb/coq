...
(* let reify_tc_opt =
   {
     optdepr = false;
     optname = "Aggressive reification using type-classes resolution";
     optkey  = ["Lia"; "Reify" ; "More"];
     optread = (fun () -> !reify_tc);
     optwrite = (fun x -> reify_tc := x)
   } in
 *)
 ...
 (* let () = declare_bool_option reify_tc_opt in*)
 ()
 ...
 *)
  ...
  let parse_formula gl parse_atom env tg term =
    ...
    let parse_atom env tg t =
      ...
      with ParseError -> (Mc.X(t),env,tg) in
